...
pygame.display.set_caption('KSP3')
...
Mars = Planet('Mars',[221,115,15], 0.3, [248,191,122], 30)
    ...
    def move(self):
        ...
        if not self.interstellar:
            if pygame.Rect.colliderect(planet.rectHitbox, (self.position[0], self.position[1], self.width, self.height)):
                ...
                gravitation = False
            if pygame.Rect.colliderect(planet.rectHitbox, (self.position[0], self.position[1]-self.speed/10, self.width, self.height)):
                if self.speed < -20:
                    World.death()
        planet.move()
